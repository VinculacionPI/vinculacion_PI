...
export default function GraduateDashboardPage() {
  ...
  const fetchOpportunities = async () => {
    setIsLoading(true)
    try {
      const response = await fetch("/api/opportunities?graduate=true")
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-foreground mb-2">Oportunidades para Graduados</h1>
        <p className="text-muted-foreground">Ofertas laborales exclusivas para egresados del TEC</p>
      </div>

      {/* Stats */}
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4 mb-8">
        <StatsCard title="Total Oportunidades" value={stats.total} icon={Briefcase} />
        <StatsCard title="Empleos" value={stats.jobs} icon={TrendingUp} />
        <StatsCard title="Aplicaciones" value={stats.applied} icon={Search} description="Postulaciones enviadas" />
        <StatsCard title="Entrevistas" value={stats.interviews} icon={Bookmark} description="En proceso" />
      </div>
